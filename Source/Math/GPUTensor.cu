...
struct TensorOpsM
{
	static __device__ ElemType ComputeM(const FixedArray<ElemType*, 2>& pointers, ElemType Gamma, ElemType gamma, ElementWiseOperator op)
	{
		ElemType a = *(pointers[0]);
#define CaseTernaryTensorOp(oper)         \
    case ElementWiseOperator::op##oper: \
        return Op##oper(a, Gamma, gamma)
		switch (op)
		{
	...
	static __device__ ElemType ComputeM(const FixedArray<ElemType*, 3>& pointers, ElemType Gamma, ElemType gamma, ElementWiseOperator op)
	{
		// const ElemType & a = *(pointers[0]);    // const & for opIndex--costs quite some code bloat
		ElemType a = *(pointers[0]);
		ElemType b = *(pointers[1]);
#define CaseQuaternaryTensorOp(oper)        \
    case ElementWiseOperator::op##oper: \
        return Op##oper(a, b, Gamma, gamma)
		switch (op)
		{
			ForAllQuaternaryOps(CaseQuaternaryTensorOp); // note: this costs about 6% compared to having only a single case
		default:
			return 0; // (failure)
		}
	}
...
template <class ElemType, C_size_t N, C_int M, C_int m>
struct TensorOpReduceM
{
	// this version for m >= 0
	static __device__ ElemType ComputeM(FixedArray<ElemType*, N> pointers, ElemType Gamma, ElemType gamma, ElementWiseOperator op,
		const FixedArray<C_unsigned_int, M>& reducingOpDims, const FixedMatrix<C_int, N, M>& reducingStrides)
	{
		// start with index 0
		// We may use 'double' since we are memory-bound anyway.
		ReduceElemType aggregate = TensorOpReduceM<ElemType, N, M, m - 1>::ComputeM(pointers, Gamma, gamma, op, reducingOpDims, reducingStrides);
		// apply this index to the pointers
		C_size_t dim = reducingOpDims[m];
		for (C_size_t k = 1 /*done with k=0 already*/; k < dim; k++)
		{
			...
			for (C_size_t i = 0; i < N - 1; i++) // N-1 because output is not used here
				pointers[i] += reducingStrides(i, (C_size_t)m);
			ElemType val = TensorOpReduceM<ElemType, N, M, m - 1>::ComputeM(pointers, Gamma, gamma, op, reducingOpDims, reducingStrides);
			aggregate += val;
		}
		return (ElemType)aggregate;
	}
...
// The pointers are pointing to the input element.
template <class ElemType, C_size_t N, C_int M>
struct TensorOpReduceM<ElemType, N, M, /*m=*/-1>
{
	// this version for m = -1
	// the pointers are pointing to the right location(s) to take the operation over
	static __device__ ElemType ComputeM(FixedArray<ElemType*, N> pointers, ElemType Gamma, ElemType gamma, ElementWiseOperator op,
		const FixedArray<C_unsigned_int, M>& /*reducingOpDims*/, const FixedMatrix<C_int, N, M>& /*reducingStrides*/)
	{
		return TensorOpsM<ElemType>::ComputeM(pointers, Gamma, gamma, op); // finally computing something!
	}
};
// -----------------------------------------------------------------------
...
template <class ElemType, C_size_t N, C_int M, C_int m>
struct TensorOpParallelReduceM
{
	// this version for m >= 0
	static __device__ ElemType ComputeM(CUDA_LONG id, FixedArray<ElemType*, N> pointers, ElemType Gamma, ElemType gamma, ElementWiseOperator op,
		const FixedArray<C_unsigned_int, M>& reducingOpDims, const FixedMatrix<C_int, N, M>& reducingStrides)
	{
		// map id (location on grid) to index[k]
		C_size_t stride = 1; // compute the stride. This seems expensive, but since we we only currently support M <= 2, this is just compile-time selection between 1 and reducingOpDims[0].
		...
		for (C_size_t i = 0; i < N - 1; i++)
			pointers[i] += index * reducingStrides(i, (C_size_t)m); // now this dimension is taken care of
		return TensorOpParallelReduceM<ElemType, N, M, m - 1>::ComputeM(id, pointers, Gamma, gamma, op, reducingOpDims, reducingStrides);
	}
};
...
// The pointers are pointing to the input element.
template <class ElemType, C_size_t N, C_int M>
struct TensorOpParallelReduceM<ElemType, N, M, /*m=*/-1>
{
	// this version for m = -1
	// the pointers are pointing to the right location(s) to take the operation over
	static __device__ ElemType ComputeM(CUDA_LONG /*id*/, FixedArray<ElemType*, N> pointers, ElemType Gamma, ElemType gamma, ElementWiseOperator op,
...
// The 'pointers' only refer to a single element, so we will bump them in-place to perform indexing.
template <class ElemType, C_size_t N, C_int M, C_int K, bool parallelReduce, C_int k>
struct TensorOpElementM
{
	// template-recursive version loops over indices
	static __device__ void ComputeM(CUDA_LONG id, ElemType beta, FixedArray<ElemType*, N>& pointers, ElemType Gamma, ElemType gamma, ElemType alpha, ElementWiseOperator op,
		const FixedArray<C_unsigned_int, K>& regularOpStrides, const FixedMatrix<C_int, N, K>& regularStrides,
		const FixedArray<C_unsigned_int, M>& reducingOpDims, const FixedMatrix<C_int, N, M>& reducingStrides,
		CUDA_LONG reductionBegin, CUDA_LONG reductionChunkSize)
	{
		...
		for (C_size_t i = 0; i < N; i++)
			pointers[i] += index * regularStrides(i, (C_size_t)k); // now this dimension is taken care of
		// process the previous index
		TensorOpElementM<ElemType, N, M, K, parallelReduce, k - 1>::ComputeM(id, beta, pointers, Gamma, gamma, alpha, op, regularOpStrides, regularStrides, reducingOpDims, reducingStrides, reductionBegin, reductionChunkSize);
	}
};
...
// specialization for k=0 where op stride is guaranteed to be 1
template <class ElemType, C_size_t N, C_int M, C_int K, bool parallelReduce>
struct TensorOpElementM<ElemType, N, M, K, parallelReduce, /*k=*/0>
{
	// template-recursive version loops over indices
	static __device__ void ComputeM(CUDA_LONG id, ElemType beta, FixedArray<ElemType*, N>& pointers, ElemType Gamma, ElemType gamma, ElemType alpha, ElementWiseOperator op,
		const FixedArray<C_unsigned_int, K>& regularOpStrides, const FixedMatrix<C_int, N, K>& regularStrides,
		...
		CUDA_LONG reductionBegin, CUDA_LONG reductionChunkSize)
	{
		...
		for (C_size_t i = 0; i < N; i++)
			pointers[i] += index * regularStrides(i, 0); // now this dimension is taken care of
		// process the previous index
		TensorOpElementM<ElemType, N, M, K, parallelReduce, -1>::ComputeM(/*id*/ 0, beta, pointers, Gamma, gamma, alpha, op, regularOpStrides, regularStrides, reducingOpDims, reducingStrides, reductionBegin, reductionChunkSize);
	}
};
...
// specialization for k = -1 terminates the template recursion, and computes reductions in a for loop
template <class ElemType, C_size_t N, C_int M, C_int K>
struct TensorOpElementM<ElemType, N, M, K, /*parallelReduce=*/false, /*k=*/-1>
{
	// template-recursion-teminating version computes the actual value for this output location
	// now the output pointers point to the right element (input pointers may still iterate for reduction)
	static __device__ void ComputeM(CUDA_LONG /*id*/, ElemType beta, FixedArray<ElemType*, N>& pointers, ElemType Gamma, ElemType gamma, ElemType alpha, ElementWiseOperator op,
...
// specialization for k = -1 terminates the template recursion, and computes reductions in parallel
template <class ElemType, C_size_t N, C_int M, C_int K>
struct TensorOpElementM<ElemType, N, M, K, /*parallelReduce=*/true, /*k=*/-1>
{
	// template-recursion-teminating version computes the actual value for this output location
	// now the output pointers point to the right element (input pointers may still iterate for reduction)
	static __device__ void ComputeM(CUDA_LONG /*id*/, ElemType beta, FixedArray<ElemType*, N>& pointers, ElemType Gamma, ElemType gamma, ElemType alpha, ElementWiseOperator op,
		...
		const FixedArray<C_unsigned_int, M>& reducingOpDims, const FixedMatrix<C_int, N, M>& reducingStrides, CUDA_LONG reductionBegin, CUDA_LONG reductionChunkSize)
	{
		...
		for (CUDA_LONG redId = reductionBegin + tid; redId < reductionEnd; redId += tids)
		{
			auto val = TensorOpParallelReduceM<ElemType, N, M, M - 1>::ComputeM(redId, pointers, Gamma, gamma, op, reducingOpDims, reducingStrides);
			sum += val;
		}
		__shared__ ReduceElemType accumulators[GridDim::maxThreadsPerBlock /*tids*/];
		accumulators[tid] = sum;
...
// launch tensor op with CUDA
template <class ElemType, C_size_t N, C_int M, C_int K>
__global__ void _launchTensorOpM(ElemType beta, FixedArray<ElemType*, N> pointers, ElemType Gamma, ElemType gamma, ElemType alpha, ElementWiseOperator op,
	FixedArray<C_unsigned_int, K> regularOpStrides, FixedMatrix<C_int, N, K> regularStrides, CUDA_LONG numElements,
	FixedArray<C_unsigned_int, M> reducingOpDims, FixedMatrix<C_int, N, M> reducingStrides)
{
	CUDA_LONG id = GridDim::GetLinearThreadId();
	if (id < numElements) // note: there are no __syncthread() calls inside
		TensorOpElementM<ElemType, N, M, K, false, K - 1>::ComputeM(id, beta, pointers, Gamma, gamma, alpha, op, regularOpStrides, regularStrides, reducingOpDims, reducingStrides, 0, 0);
}
...
template <class ElemType, C_size_t N, C_int K>
static void LaunchTensorOpM(ElemType beta, array<ElemType*, N> pointerVector, ElemType Gamma, ElemType gamma, ElemType alpha, ElementWiseOperator op,
	const SmallVector<size_t>& regularOpDims, const array<SmallVector<ptrdiff_t>, N>& regularStrideVectors)
{
	// copy all parameters to CUDA-compatible data structures
	FixedArray<ElemType*, N> pointers(pointerVector);
	...
	SyncGuard syncGuard;
	GridDim grid(NN);
	_launchTensorOpM<ElemType, N, /*M=*/0, K> << <grid.m_blocksPerGrid, grid.m_threadsPerBlock, 0, t_stream >> >(beta, pointers, Gamma, gamma, alpha, op, regularOpStrides, regularStrides, grid.m_N, reducingOpDims, reducingStrides);
}
...
template <class ElemType, C_size_t N, C_int M, C_int K>
__global__ void _launchTensorOpWithReductionM(ElemType beta, FixedArray<ElemType*, N> pointers, ElemType Gamma, ElemType gamma, ElemType alpha, ElementWiseOperator op,
	FixedArray<C_unsigned_int, K> regularOpStrides, FixedMatrix<C_int, N, K> regularStrides, CUDA_LONG numElements,
	FixedArray<C_unsigned_int, M> reducingOpDims, FixedMatrix<C_int, N, M> reducingStrides, CUDA_LONG reductionBegin, CUDA_LONG reductionChunkSize)
{
	CUDA_LONG id = gridDim.x * blockIdx.y + blockIdx.x; // input dimensions are Y dimension of blocks in this case, so we can use thread dim for shared-memory/parallelization
	if (id < numElements)                               // note: we have __syncthread() calls but only entire blocks in sync, so this is OK
		TensorOpElementM<ElemType, N, M, K, true, K - 1>::ComputeM(id, beta, pointers, Gamma, gamma, alpha, op, regularOpStrides, regularStrides, reducingOpDims, reducingStrides, reductionBegin, reductionChunkSize);
}
...
// All dimensions (N-ariness, number of input dimensions K and number of reduction dimensions M) are bound to template parameters now.
template <class ElemType, C_size_t N, C_int M, C_int K>
static void LaunchTensorOpWithReductionM(ElemType beta, array<ElemType*, N> pointerVector, ElemType Gamma, ElemType gamma, ElemType alpha, ElementWiseOperator op,
	const SmallVector<size_t>& regularOpDims, const array<SmallVector<ptrdiff_t>, N>& regularStrideVectors,
	const SmallVector<size_t>& reducingOpDimVector, const array<SmallVector<ptrdiff_t>, N>& reducingStrideVectors)
{
	// copy all parameters to CUDA-compatible data structures
	...
	if (reductionDim > 1 && grid.m_blocksPerGrid < props.multiProcessorCount /*    && NN == 10 && reductionDim <= GridDim::maxThreadsPerBlock*/)
	{
		...
		if (beta == 1 || numBlocksZ == 1)
		{
			_launchTensorOpWithReductionM<ElemType, N, M, K> << <dim3(numBlocksX, numBlocksY, numBlocksZ), numThreadsX, numThreadsX * sizeof(ReduceElemType), t_stream >> >(beta, pointers, Gamma, gamma, alpha, op, regularOpStrides, regularStrides, NN, reducingOpDims, reducingStrides, 0, reductionChunkSize);
		}
		else
		{
			// We need more than one chunk, we will use atomicAdd().
			// First reset/pre-multiply input; then do the remaining chunks using atomicAdd().
			_launchTensorOpWithReductionM<ElemType, N, M, K> << <dim3(numBlocksX, numBlocksY, 1), numThreadsX, numThreadsX * sizeof(ReduceElemType), t_stream >> >(beta, pointers, Gamma, gamma, alpha, op, regularOpStrides, regularStrides, NN, reducingOpDims, reducingStrides, 0, reductionChunkSize);
			_launchTensorOpWithReductionM<ElemType, N, M, K> << <dim3(numBlocksX, numBlocksY, numBlocksZ - 1), numThreadsX, numThreadsX * sizeof(ReduceElemType), t_stream >> >(/*beta=*/1, pointers, Gamma, gamma, alpha, op, regularOpStrides, regularStrides, NN, reducingOpDims, reducingStrides, reductionChunkSize, reductionChunkSize);
		}
	}
	else
	{
		// we got enough elements to generate: do one element per thread, and reduction inside
		_launchTensorOpM<ElemType, N, M, K> << <grid.m_blocksPerGrid, grid.m_threadsPerBlock, 0, t_stream >> >(beta, pointers, Gamma, gamma, alpha, op, regularOpStrides, regularStrides, grid.m_N, reducingOpDims, reducingStrides);
	}
}
...
template <class ElemType, C_size_t N, C_int K>
static void TensorOpWithRegularLoopM(ElemType beta, const array<ElemType*, N>& pointers, ElemType Gamma, ElemType gamma, ElemType alpha, ElementWiseOperator op,
	const SmallVector<size_t>& regularOpDims, const array<SmallVector<ptrdiff_t>, N>& regularStrides,
	const SmallVector<size_t>& reducingOpDims, const array<SmallVector<ptrdiff_t>, N>& reducingStrides)
{
	size_t dims = reducingOpDims.size();
	...
	{
	case 2:
		return LaunchTensorOpWithReductionM<ElemType, N, 2, K>(beta, pointers, Gamma, gamma, alpha, op, regularOpDims, regularStrides, reducingOpDims, reducingStrides);
	case 1:
		return LaunchTensorOpWithReductionM<ElemType, N, 1, K>(beta, pointers, Gamma, gamma, alpha, op, regularOpDims, regularStrides, reducingOpDims, reducingStrides);
	case 0:
		return LaunchTensorOpM<ElemType, N, K>(beta, pointers, Gamma, gamma, alpha, op, regularOpDims, regularStrides);
	default:
		LogicError("TensorOp: %d non-flattened reduction dimensions are not supported.", (C_int)dims);
	}
}
	...
	const SmallVector<size_t>& reducingOpDims, const array<SmallVector<ptrdiff_t>, N>& reducingStrides)
{
	...
	{
	case 4:
		return TensorOpWithRegularLoopM<ElemType, N, 4>(beta, pointers, Gamma, gamma, alpha, op, regularOpDims, regularStrides, reducingOpDims, reducingStrides);
	case 3:
		return TensorOpWithRegularLoopM<ElemType, N, 3>(beta, pointers, Gamma, gamma, alpha, op, regularOpDims, regularStrides, reducingOpDims, reducingStrides);
	case 2:
		return TensorOpWithRegularLoopM<ElemType, N, 2>(beta, pointers, Gamma, gamma, alpha, op, regularOpDims, regularStrides, reducingOpDims, reducingStrides);
	case 1:
		return TensorOpWithRegularLoopM<ElemType, N, 1>(beta, pointers, Gamma, gamma, alpha, op, regularOpDims, regularStrides, reducingOpDims, reducingStrides);
	case 0:
		return TensorOpWithRegularLoopM<ElemType, N, 0>(beta, pointers, Gamma, gamma, alpha, op, regularOpDims, regularStrides, reducingOpDims, reducingStrides);
	default:
		LogicError("TensorOp: %d non-flattened input dimensions are not supported. M-type", (C_int)dims);
	}
}

template void TensorOpM<float, 2>(float beta, array<float*, 2> pointers, float b, float c, float alpha, ElementWiseOperator op,
	const array<size_t, 2>& offsets,
	const SmallVector<size_t>& regularOpDims, const array<SmallVector<ptrdiff_t>, 2>& regularStrides,
	const SmallVector<size_t>& reducingOpDims, const array<SmallVector<ptrdiff_t>, 2>& reducingStrides);
template void TensorOpM<float, 3>(float beta, array<float*, 3> pointers, float c, float d, float alpha, ElementWiseOperator op,
	const array<size_t, 3>& offsets,
	const SmallVector<size_t>& regularOpDims, const array<SmallVector<ptrdiff_t>, 3>& regularStrides,
	const SmallVector<size_t>& reducingOpDims, const array<SmallVector<ptrdiff_t>, 3>& reducingStrides);
template void TensorOpM<double, 2>(double beta, array<double*, 2> pointers, double b, double c, double alpha, ElementWiseOperator op,
	const array<size_t, 2>& offsets,
	const SmallVector<size_t>& regularOpDims, const array<SmallVector<ptrdiff_t>, 2>& regularStrides,
	const SmallVector<size_t>& reducingOpDims, const array<SmallVector<ptrdiff_t>, 2>& reducingStrides);
template void TensorOpM<double, 3>(double beta, array<double*, 3> pointers, double b, double c, double alpha, ElementWiseOperator op,
	const array<size_t, 3>& offsets,
	const SmallVector<size_t>& regularOpDims, const array<SmallVector<ptrdiff_t>, 3>& regularStrides,
	const SmallVector<size_t>& reducingOpDims, const array<SmallVector<ptrdiff_t>, 3>& reducingStrides);
