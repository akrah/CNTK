...
#pragma push_macro("OverloadUnaryMathFns")
			...
			template <class ElemType>
			DECL ElemType factorial(ElemType n)
			{
				ElemType ret = 1;
			...
			template <class ElemType>
			DECL ElemType combinations(ElemType n, ElemType k)
			{
				return (factorial(n) / (factorial(k) * factorial(n - k)));
			}

			template <class ElemType>
			DECL ElemType Autobinomial(ElemType z, ElemType Gamma, ElemType gamma)
			{
				ElemType sigma = Sigmoid(z);
				ElemType Sigma = 0;
				
				for (ElemType i = 0; i <= gamma; i++)
					Sigma += combinations(Gamma, i) * pow(sigma, i) * pow((1 - sigma), (Gamma - i));
				return z;
			}

			template <class ElemType>
			DECL ElemType AutobinomialDerivative(ElemType z, ElemType Gamma, ElemType gamma)
			{
				ElemType sigma = Sigmoid(z);
				ElemType d_sigma = SigmoidDerivative(z);
				ElemType Sigma = 0;
				...
				for (ElemType i = 0; i <= gamma; i++)
				{
					ElemType pi = Autobinomial(z, Gamma, i);
					Sigma += pi * (i - Gamma * sigma);
				}
...
#define DefTernaryOp(op, expr)                               \
    ...
    DECL ElemType Op##op(ElemType a, ElemType b, ElemType c) \
        ...
        return expr;                                         \
			...
			DefTernaryOp(Autobinomial, Autobinomial(a, b, c));
			DefTernaryOp(AutobinomialDerivative, AutobinomialDerivative(a, b, c));
